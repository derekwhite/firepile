...
//            println("SCALASIG FIELD: " + f.fieldTyp)
 ...
 // Bin(op1, "<<", op2)
        ...
        */
        ...
        case GNeg(op) => Apply(Select(op,
                                            Method(translateTypeSimpleName(op.getType)+".$neg",MethodType(List(LocalValue(NoSymbol,"x",translateType(op.getType))),
                                                                       translateType(op.getType)))), Nil) //DummyTree("Un(-)")  // Un("-", op)
        case _ => null
      }
    ...
    v match {
      ...
      case _ => {
        ...
        v match {
          ...
          case FirepileMathCall(name, args) => Apply(Select(Ident(NoSymbol), Method(name,MethodType(List(LocalValue(NoSymbol,"x",PrefixedType(ThisType(Class("scala")),Class("scala.Float")))),
                                                                       PrefixedType(ThisType(Class("scala")),Class("scala.Float"))))), args.map(a => translateExp(a, symtab, anonFuns)))

          //  Call(Id(name), args.map(a => translateExp(a, symtab, anonFuns)))
          ...
          case GVirtualInvoke(base, method, args) => {
            handleIdsVirtualInvoke(v, symtab, anonFuns) match {
              ...
              case _ => {
                ...
                Apply(Select(Ident(NoSymbol), Method(method.name,MethodType(List(LocalValue(NoSymbol,"x",PrefixedType(ThisType(Class("scala")),Class("scala.Any")))),
                                                                       PrefixedType(ThisType(Class("scala")),Class("scala.Any"))))), args.map(a => translateExp(a, symtab, anonFuns)))
              }
            }
          }
...
*/
  ...
  private def handleIdsVirtualInvoke(v: Value, symtab: SymbolTable, anonFuns: HashMap[String, Value])(implicit currentUnit: SootUnit): Option[Tree] = {
    ...
    // println("handleIdsVirtualInvoke: fieldName: " + fieldName)
