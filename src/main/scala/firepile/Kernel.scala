...
object Kernel {
  ...
  def printMemUse = {
    println("Number of global items: " + globalSize)
    println("Heap memory: " + heapMemory)
  }
  ...
  val heapArgs = new ArrayList[(String, SootType, Int, Int, Int)]()       /* (name, type, index, size, array len) */
